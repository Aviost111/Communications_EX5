...
def packet_callback(packet):
    ...
    if IP in packet:
        ...
        source_port = dest_port = data = cache_flag = steps_flag = type_flag = status_code = cache_control = None

        # Check the protocol number and extract relevant information accordingly
        if TCP in packet:  # TCP
            source_port = packet[TCP].sport
            dest_port = packet[TCP].dport
            data = hexstr(packet[TCP].payload)
            cache_flag = packet[TCP].options
            steps_flag = packet[TCP].flags.S
            type_flag = packet[TCP].sprintf("%TCP.flags%")
            ...
            if HTTPResponse in packet:
                ...
                status_code = http_layer.Status_Code
                cache_control = http_layer.Cache_Control
            # Create a dictionary with the extracted information as key-value pairs
            info_dict = {
                "source_ip": source_ip,
                "dest_ip": dest_ip,
            ...
            # Write the dictionary to an output file in append mode using json.dumps function
            with open('output.txt', 'a+') as f:
            ...
            source_port = packet[UDP].sport
            dest_port = packet[UDP].dport
            data = hexstr(packet[UDP].payload)

        elif ICMP in packet:  # ICMP
            data = hexstr(packet[ICMP].payload)

        elif IGMP in packet:  # IGMP
            data = hexstr(packet[IGMP].payload)

        elif Raw in packet:  # RAW
            data = hexstr(packet[Raw].payload)
...
if __name__ == "__main__":
    try:
        ...
        print("Starting sniffer...")
        # Start sniffing packets and call the packet_callback function for each captured packet
        sniff(filter="tcp", prn=packet_callback, iface="lo")
    except Exception as e:
        # Catch any exceptions that might occur and print the error message
        print(f"Sniffer error: {e}")
        sys.exit(1)
